...
#[command(name = "meta")]
#[command(
    about = "A lightweight tool to copy file contents (ASCII text only) or directory structure content (meta) to/from clipboard"
)]
struct Cli {
    ...
    if abs.is_file() {
        ...
        match get_file_list(&abs) {
            Ok(list) => {
                ...
                println!(
                    "✅ Copied directory structure of {} to clipboard",
                    abs.display()
                );
            }
            Err(e) => {
            ...
            if is_dir {
                ...
                if let Some(parent) = full_path.parent() {
                    if let Err(e) = fs::create_dir_all(parent) {
                        eprintln!(
                            "Error creating parent dirs for '{}': {}",
                            full_path.display(),
                            e
                        );
                        exit(1);
    ...
    for (idx, ent) in entries.iter().enumerate() {
        ...
        if ent.is_dir {
            eprintln_v(
                opts.verbose,
                2,
                format!("[DEBUG] mkdir: {}", path.display()),
            );
            ...
            eprintln_v(
                opts.verbose,
                2,
                format!("[DEBUG] touch: {}", path.display()),
            );
